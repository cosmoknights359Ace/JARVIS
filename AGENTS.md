...
- **2026-09-06 — Backend/provider split (refactor)** — Inference logic extracted out of `jarvis_gui.py` into two GUI-agnostic modules: `jarvis_backend.py` (model state/routing, memory store, request assembly, OS actions) and `jarvis_providers.py` (local Ollama + OpenAI-compatible cloud). The GUI now imports `jarvis_backend as backend` and `jarvis_providers as providers` and calls `backend.build_system_prompt/build_request_messages/open_provider_stream`, `providers.local_chat_stream/local_vision_analyze/check_ollama/check_cloud`, and `backend.ModelState` for routing. The Ollama UI-freeze fix is preserved: all local calls go through `_local_client(timeout=(3.0, None, None, None))` (fast connect fail, unbounded read) so a dead/unreachable server can't hang the UI thread. `jarvis_gui.pyw` kept byte-identical to `jarvis_gui.py`.
 - **2026-09-08 — Attribution/credit-protection layer** — Added `attribution.py` as the single source of truth for J.A.R.V.I.S creator credit (vinod, 2026). The notice is fragmented (base64-encoded + plaintext fragments) and reconstructed at runtime by `attribution.get_notice()` / `get_short()`. Boot paths call `attribution.stamp_log("jarvis.log")` so the credit is re-written into the action log on every run (removal is fragile), and `attribution.integrity_ok()` detects a missing/altered notice to surface a warning. Wired into `jarvis_gui.py` (window titles, greeting/help footers, boot stamp + integrity check, export-chat attribution), `jarvis.py` (CLI banner + boot stamp + integrity warning), `jarvis_backend.py` (HELP_TEXT credits footer), and `jarvis_providers.py` (import-time log stamp). License is **permissive**: free to use/modify/redistribute — but the credit MUST be retained on reuse/fork. Do not strip the attribution layer when editing these files.
